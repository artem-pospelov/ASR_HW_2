...
class Trainer(BaseTrainer):
    ...
    def log_predictions(
        ...
        argmax_inds = [inds[: int(ind_len)] for inds, ind_len in zip(probs.argmax(-1), probs_length.numpy())]
        probs = [prob[:length, :] for prob, length in zip(probs, probs_length.numpy())]
        argmax_texts_raw = [self.text_encoder.decode(inds) for inds in argmax_inds]
        argmax_texts = [self.text_encoder.ctc_decode(inds) for inds in argmax_inds]
        bs_lm_texts = [self.text_encoder.ctc_beam_search(prob).get("hyp_text") for prob in probs]
        tuples = list(zip(argmax_texts, bs_lm_texts, text, argmax_texts_raw, audio_path))
